...
| `mcr run-rootfs node-rootfs /bin/sh -c "<run JavaScript with node>"` | Node.js JavaScript execution is repeatably green locally after the native execution fixes. The extended smoke now runs a real `node -e` script instead of only `node -v`. | Keep broader Node package-manager/build workloads on the workload backlog; the extended-support JavaScript smoke is restored. |
| `mcr run-rootfs jdk-rootfs /bin/sh -c "<javac then java>"` | JDK compile-and-run is repeatably green locally when the smoke pins HotSpot to interpreted mode with `javac -J-Xint` and `java -Xint`. Unpinned `javac -version` still runs into a long clock/futex polling path. | Keep the unpinned HotSpot JIT/scheduler performance loop on the workload backlog, but the extended-support compile-and-run smoke is restored. |
| `mcr run-rootfs mysql-rootfs /bin/sh -c "<bootstrap mariadbd and create database>"` | MariaDB bootstrap mode now initializes InnoDB and executes SQL that creates a database in the extended smoke. Full install-db plus background daemon/client query remains broader than the restored smoke and can still block on shell pipe/startup behavior. | Keep full local server startup plus client query coverage on the workload backlog; the extended-support bootstrap database smoke is restored. |

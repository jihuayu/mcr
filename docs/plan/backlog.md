...
| `mcr run-rootfs node-rootfs /bin/sh -c "cd /tmp/mcr-next-smoke && node probe-config-require.js"` | The focused Next.js config-loader probe now passes through `nextjs_config_loader_fs_tls_contract` after `jit-003` classified every FS-relative site and trapped the V8 TLS forms that cannot be safely materialized. | Keep full `next build` as the remaining larger workload proof; the config-loader FS/TLS regression is closed. |

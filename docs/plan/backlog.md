...
| `mcr run-rootfs mysql-rootfs /bin/sh -c "<bootstrap mariadbd, create table, insert row>"` | MariaDB bootstrap mode now initializes InnoDB and executes SQL that creates a database, creates an InnoDB table, inserts a row, and verifies table artifacts in the extended smoke. Full install-db plus background daemon/client query remains broader than the restored smoke; the latest bounded install-db probe still did not complete. | Keep full local server startup plus client query coverage on the workload backlog; the extended-support bootstrap table smoke is restored. |

...
| `mcr run-rootfs node-rootfs /bin/sh -c "node -v"` | `node -v` is repeatably green locally, but `node -e ""` / `node -e "console.log('node-ok')"` still faults at musl `hlt` in the native execution path. | Stabilize the remaining native/JIT block execution path before promoting the extended smoke back from binary execution to JavaScript execution. |
| `mcr run-rootfs jdk-rootfs /bin/sh -c "<javac then java>"` | The JDK fixture path issue is fixed, but running the JVM reaches a no-runnable futex stall with two tasks waiting for futex wake. The extended smoke is temporarily bounded to a JDK native helper plus java/javac presence. | Diagnose the JVM startup futex handshake and restore compile-and-run coverage once the scheduler/futex path is correct. |
| `mcr run-rootfs mysql-rootfs /bin/sh -c "<start mysqld and query>"` | MariaDB server/client version commands run. The original startup contract now gets past the prefixed FS-relative native fault and reaches a JIT/exception terminator during install/startup. | Classify the MariaDB install/startup exception terminator and restore local server query coverage after the native execution gap is fixed. |

...
| FS/TLS native execution still has a correctness gap if coverage depends on narrow fixed-width rewrites plus host fault fallback. | Decode every FS-relative memory operand, materialize only proven-safe forms, and force all remaining sites through an explicit trap or interpreter marker before host native execution. | `jit-003` |

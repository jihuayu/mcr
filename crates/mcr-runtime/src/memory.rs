...
impl GuestMemoryMap {
    ...
    pub fn from_image(image: &GuestMemoryImage) -> Result<Self, GuestMemoryError> {
        let mut memory = Self::new(align_up(image.brk())?)?;
        for vma in image.vmas() {
            let protection = GuestMemoryProtection::new(
                vma.permissions().read(),
                vma.permissions().write(),
                vma.permissions().execute(),
            );
            let kind = match vma.kind() {
                ElfGuestVmaKind::ElfLoad { .. } => GuestVmaKind::ElfLoad,
                ElfGuestVmaKind::Stack => GuestVmaKind::Stack,
            };
            memory.insert_mapping(vma.start(), vma.end() - vma.start(), protection, kind)?;
        }
        for region in image.regions() {
            memory.write(region.start(), region.bytes())?;
        }
        Ok(memory)
    }
    ...
    fn insert_mapping(
        ...
        let memory =
            HostMemory::allocate(len, protection.to_host()).map_err(GuestMemoryError::Host)?;
...
impl crate::GuestMemory for GuestMemoryMap {
    fn read_bytes(
        &self,
        addr: GuestAddress,
        buffer: &mut [u8],
    ) -> Result<(), crate::GuestMemoryError> {
        self.read(addr, buffer)
            .map_err(|error| crate::GuestMemoryError::Fault(error.errno()))
    }
...
impl MemorySyscalls for GuestMemoryMap {

...
fn socket_address_from_linux(address: LinuxSocketAddress) -> SocketAddress {
    match address {
        LinuxSocketAddress::Unix { path, len } => SocketAddress::unix(&path[..usize::from(len)])
            .expect("decoded UNIX socket address length is valid"),
...
fn socket_address_to_linux(address: SocketAddress) -> LinuxSocketAddress {
    match address {
        SocketAddress::Unix { path, len } => LinuxSocketAddress::unix(&path[..usize::from(len)])
            .expect("runtime UNIX socket address length is valid"),

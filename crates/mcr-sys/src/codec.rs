...
use crate::{
    LINUX_AF_INET, LINUX_AF_INET6, LINUX_AF_UNIX, LINUX_POLLIN, LINUX_POLLOUT, LINUX_POLLPRI,
    LINUX_POLLRDNORM, LINUX_POLLWRNORM, LinuxErrno, LinuxIovec, LinuxMsghdr, LinuxPollfd,
    LinuxTimespec,
};
...
pub const LINUX_SOCKADDR_UN_PATH_LEN: usize = 108;
pub const LINUX_SOCKADDR_UN_LEN: usize = 2 + LINUX_SOCKADDR_UN_PATH_LEN;
...
pub enum LinuxSocketAddress {
    Unix {
        path: [u8; LINUX_SOCKADDR_UN_PATH_LEN],
        len: u8,
    },
    Inet {
        address: [u8; 4],
...
impl LinuxSocketAddress {
    #[must_use]
    pub fn unix(path: &[u8]) -> Result<Self, LinuxErrno> {
        if path.len() > LINUX_SOCKADDR_UN_PATH_LEN {
            return Err(LinuxErrno::EINVAL);
        }
        let mut stored = [0; LINUX_SOCKADDR_UN_PATH_LEN];
...
pub fn read_socket_address(
    ...
    match u32::from(u16::from_le_bytes(family)) {
        LINUX_AF_UNIX => {
            let read_len = (addrlen as usize).min(LINUX_SOCKADDR_UN_LEN);
            let mut bytes = [0; LINUX_SOCKADDR_UN_LEN];
            memory
            ...
            LinuxSocketAddress::unix(&path[..path_len])
        }
        LINUX_AF_INET => {
...
pub fn encode_socket_address(address: LinuxSocketAddress) -> Vec<u8> {
    match address {
        LinuxSocketAddress::Unix { path, len } => {
            let path_len = usize::from(len);
            let terminator_len = usize::from(
                path_len < LINUX_SOCKADDR_UN_PATH_LEN && path.first().copied() != Some(0),
            );
...
mod tests {
    ...
    fn sockaddr_codec_round_trips_ipv4_and_ipv6() {
        let unix = LinuxSocketAddress::unix(b"/tmp/mcr.sock").unwrap();
        let encoded_unix = encode_socket_address(unix);
        assert_eq!(
            encoded_unix,
            [
                1, 0, b'/', b't', b'm', b'p', b'/', b'm', b'c', b'r', b'.', b's', b'o', b'c', b'k',
                0
            ]
        );

        let mut memory = TestMemory::default();
        memory.write_raw(0x800, &encoded_unix);
        assert_eq!(
            read_socket_address(&memory, 0x800, encoded_unix.len() as u32).unwrap(),
            unix
        );

        let abstract_unix = LinuxSocketAddress::unix(b"\0mcr").unwrap();
        let encoded_abstract = encode_socket_address(abstract_unix);
        assert_eq!(encoded_abstract, [1, 0, 0, b'm', b'c', b'r']);
        memory.write_raw(0x900, &encoded_abstract);
        assert_eq!(
            read_socket_address(&memory, 0x900, encoded_abstract.len() as u32).unwrap(),
            abstract_unix
        );
